...
Web3.prototype.define = function(modelData) {
  ...
  var constructor = modelData.settings.ethereum.constructor;
  var contractStr = getSolSourceFile(modelData.settings.ethereum.sol);
  var compiledContract = compileSolSourceFile(contractStr);
  var abi = getAbi(compiledContract, constructor.name);
  //var abi = getAbi(compiledContract, constructor);
  var bytecode = getBytecode(compiledContract, constructor.name);
  var Contract = web3.eth.contract(abi);

  model.construct = function(data, cb) {
    var params = data.args['params'].toString();
    var contract = Contract.new(params,{
      from: account,
      data: bytecode.toString(),
      gas: modelData.ethereum.gas
    ...
    if (obj.constant === false && obj.type === "function") {
      model.prototype[obj.name] = function(data, cb) {
        ...
        obj.inputs.map(function(input) {
          return data.args[input.name];
        })
      }
      model.remoteMethod(obj.name, {
        description: 'Call the ' + obj.name + ' contract method',
        accepts: {arg: 'data', type: obj.type, http: {source: 'body'}},
